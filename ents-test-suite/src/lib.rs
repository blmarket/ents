...
use ents::{
    DatabaseError, EdgeQuery, EdgeValue, EntExt, QueryEdge, ReadEnt,
    TransactionProvider, Transactional,
};
use ents_admin::{AdminEnt, AuditError};

pub fn test_basic_create<R: TransactionProvider>(r: &R) -> anyhow::Result<()> {
    println!("  Testing basic create...");

    let id = r.execute(|txn| {
...
pub fn test_relationships<R: TransactionProvider>(r: &R) -> anyhow::Result<()> {
    println!("  Testing relationships (User-Post-Tag)...");

    let (user_id, tag1_id, tag2_id, tag3_id, post_id) =
        r.execute(|txn| {
            // Create a user
            let user = User::new(
                "johndoe".to_string(),
            ...
            Ok::<_, DatabaseError>((
                user_id, tag1_id, tag2_id, tag3_id, post_id,
            ))
        })??;
...
pub fn test_unique_constraints<R: TransactionProvider>(
    r: &R,
) -> anyhow::Result<()> {
    println!("  Testing UNIQUE constraints...");

    let result: anyhow::Result<()> = r.execute(|txn| {
...
pub fn run_all_tests<R: TransactionProvider>(runner: R) -> anyhow::Result<()> {
...
pub fn test_basic_read<R: TransactionProvider>(r: &R) -> anyhow::Result<()> {
...
pub fn test_basic_update<R: TransactionProvider>(r: &R) -> anyhow::Result<()> {
...
pub fn test_basic_delete<R: TransactionProvider>(r: &R) -> anyhow::Result<()> {
...
pub fn test_error_handling<R: TransactionProvider>(
    r: &R,
) -> anyhow::Result<()> {
...
pub fn test_multiple_entities<R: TransactionProvider>(
    r: &R,
) -> anyhow::Result<()> {
    println!("  Testing multiple entities...");

    let ids = r.execute(|txn| {
        let mut ids = Vec::new();
        for i in 0..5 {
...
pub fn test_concurrent_updates<R: TransactionProvider>(
    ...
    let (entity_data, last_updated) = r.execute(|txn| {
    ...
    for i in 0..3 {
        ...
        let result: anyhow::Result<bool> = r.execute(|txn| {
            ...
            let mut stale_entity = entity_data_clone.clone();
...
pub fn test_audit_success<R>(r: &R) -> anyhow::Result<()>
where
    R: TransactionProvider,
    for<'a> R::Tx<'a>: AdminEnt,
{
    println!("  Testing audit success case...");

    // First, create entities with proper edges
    let (user_id, tag_id, post_id) = r.execute(|txn| {
        let user =
        ...
        txn.commit()?;
        Ok::<_, DatabaseError>((user_id, tag_id, post_id))
    })??;
    ...
    r.execute(|txn| {
        ...
        assert_eq!(
            author_result.edges.len(),
            1,
            "Author edge should still exist"
        );
        assert_eq!(author_result.edges[0].dest, post_id);

        // Check tag edge: Tag --[tagged]--> Post
        let tag_result =
            txn.find_edges(tag_id, EdgeQuery::asc(&[b"tagged"]))?;
...
pub fn test_audit_entity_not_found<R>(r: &R) -> anyhow::Result<()>
where
    R: TransactionProvider,
    for<'a> R::Tx<'a>: AdminEnt,
{
...
pub fn test_audit_unexpected_entity_type<R>(r: &R) -> anyhow::Result<()>
where
    R: TransactionProvider,
    for<'a> R::Tx<'a>: AdminEnt,
{
...
pub fn test_audit_edge_mismatch_missing_edge<R>(r: &R) -> anyhow::Result<()>
where
    R: TransactionProvider,
    for<'a> R::Tx<'a>: AdminEnt,
{
    ...
    let (user_id, _tag_id, post_id) = r.execute(|txn| {
        let user = User::new(
...
pub fn test_audit_edge_mismatch_extra_edge<R>(r: &R) -> anyhow::Result<()>
where
    R: TransactionProvider,
    for<'a> R::Tx<'a>: AdminEnt,
{
    ...
    // Create entities
    let (_user_id, post_id) = r.execute(|txn| {
        let user = User::new(
        ...
        txn.commit()?;
        Ok::<_, DatabaseError>((user_id, post_id))
    })??;
...
pub fn test_audit_edge_mismatch_wrong_content<R>(r: &R) -> anyhow::Result<()>
where
    R: TransactionProvider,
    for<'a> R::Tx<'a>: AdminEnt,
{
    ...
    // Create entities
    let (_user_id, other_user_id, post_id) = r.execute(|txn| {
        let user = User::new(
        ...
        txn.commit()?;
        Ok::<_, DatabaseError>((user_id, other_user_id, post_id))
    })??;
...
pub fn test_audit_null_edge_provider<R>(r: &R) -> anyhow::Result<()>
where
    R: TransactionProvider,
    for<'a> R::Tx<'a>: AdminEnt,
{
...
pub fn test_fix_ent_edges<R>(r: &R) -> anyhow::Result<()>
where
    R: TransactionProvider,
    for<'a> R::Tx<'a>: AdminEnt,
{
...
pub fn test_list_entities<R>(r: &R) -> anyhow::Result<()>
where
    R: TransactionProvider,
    for<'a> R::Tx<'a>: AdminEnt,
{
...
pub fn run_audit_tests<R>(runner: R) -> anyhow::Result<()>
where
    R: TransactionProvider,
    for<'a> R::Tx<'a>: AdminEnt,
{
